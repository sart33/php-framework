...
/*** Указан в форме стрелочной функции ***/
    return new Promise((resolve, reject) => {

        let xhr = new XMLHttpRequest();
        // Открываем соединение
        xhr.open(set.type, set.url,true);
        // Настройки соединения вводятся после его открытия
        ...
        xhr.onload = function () {
            if (this.status >= 200 && this.status < 300) {
                ...
                if (/fatal\s+?error/ui.test(this.response)) {
                    ...
                    // После reject или resolve - никакие вызовы выполняться дальше не будут.
                    // Соответственно ничего дальше писать не нужно.
